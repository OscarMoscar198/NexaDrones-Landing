...
import Offer from "../../assets/hero/4.jpg"
import { Text } from "../atoms/Text"
import { OfferTexts } from "../particles/Data"
import { List } from "../atoms/List"
import { useCallback } from "react"
import { FilmStrip, Package, Binoculars } from "@phosphor-icons/react"
...
const Offers = () => {
    ...
    const renderIcon = useCallback((element: number) => {
        switch (element) {
            case 0:
                return <Binoculars size={50} color="currentColor" weight="light" />;
            case 1:
                return <FilmStrip size={50} color="currentColor" weight="light" />;
            case 2:
                return <Package size={50} color="currentColor" weight="light" />;
            default:
                return "";
        }
    }, [])
            ...
            <main className="w-full lg:h-[800px] grid md:grid-cols-2 items-center gap-10 md:gap-0 lg:gap-0 ">
                ...
                <div className="h-full w-full lg:px-10 px-4 flex flex-col lg:justify-center md:justify-end justify-center items-start md:gap-20 lg:gap-24 gap-16">
                    <Fade className="w-full">
                        <div className="w-full flex flex-col mt-10 lg:mt-24 items-center relative before:absolute before:-bottom-6 before:left-38 before:w-20 before:h-1 before:rounded-lg before:bg-gradient-to-r before:from-red-500 before:to-red-700 z-10">
                            <Text as="p" className="text-red-500 lg:text-sm text-xs tracking-widest uppercase font-medium">{OfferTexts.firstText}</Text>
                        ...
                        <ul className="flex flex-col lg:gap-8 gap-6 pb-16">
                            {
                                OfferTexts.list.map((item, index) => (
                                    <List className="flex lg:gap-6 gap-4 items-start justify-start" key={index}>
                                        <Text as="span" className="text-red-600 ">{renderIcon(index)}</Text>
